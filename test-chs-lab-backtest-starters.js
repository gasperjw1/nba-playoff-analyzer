...
function backtestGame(series, gameNum) {
  ...
  const trials = [];
  // Phase 73l-starters: tighter rotation filter ≥ 28 min (true starters
  // only) per user empirical observation that starter-only cross-team
  // parlays hit 2 days in a row. The default ≥18 min lets in 6th/7th
  // men whose distributions are noisier.
  const STARTER_MIN = 28;
  for (let t = 0; t < TRIALS; t++) {
    const mc = runMonteCarlo(masked, gameNum, { iterations: ITER });
    if (!mc) { trials.push({ skipped: true }); continue; }
    const reliable = buildReliableParlay(mc, masked, { minProjectedMinutes: STARTER_MIN });
    const traditional = buildTraditionalParlay(mc, masked, { minProjectedMinutes: STARTER_MIN });
...
// ── Main run ────────────────────────────────────────────────────────
console.log(`\n=== CHS LAB BACKTEST (STARTERS-ONLY) — ${ROUND_FILTER} round, ${TRIALS} trials × ${ITER} iter, min ≥ 28 ===\n`);
...
const reportPath = path.join(__dirname, 'CHS_LAB_BACKTEST_REPORT_STARTERS.md');
fs.writeFileSync(reportPath, reportText, 'utf8');

const jsonPath = path.join(__dirname, 'CHS_LAB_BACKTEST_DETAIL_STARTERS.json');
