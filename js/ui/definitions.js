...
function renderDefinitionsPage(el) {
  ...
  <div class="page-content">
    ...
    <div class="def-grid">
      ...
      <div class="def-card" style="border-left:3px solid #f39c12;"><div class="def-term">Compound Historical Scenarios <span class="def-abbr">CHS</span></div><div class="def-desc"><strong>Phase 52.</strong> Conditional-narrowing system: each player carries stacked historical conditions (role + opponent team + venue + coach + primary defender + playoff context + health). When ALL conditions match the current game, the scenario fires and adjusts projections. Multiple matching scenarios compound with 70% decay (diminishing returns). Cascade model: when a star's scenario delta is large (&gt;2pts), teammates receive compensating adjustments. 12 condition types. Safety cap: &plusmn;35% of base stats. Wired as modifier #14 in <code>calcExpectedPlayerStats</code>. Example: "Reaves vs OKC switching D + oblique injury + third option" = 4 matched conditions &rarr; -5.0pts projection (matching his 14.7ppg actual vs OKC).</div></div>
      <div class="def-card" style="border-left:3px solid #f39c12;"><div class="def-term">CHS Game Context <span class="def-abbr">CHSctx</span></div><div class="def-desc"><strong>Phase 52/53.</strong> <code>buildGameContext(player, series, gameIdx)</code> assembles the runtime values that CHS conditions evaluate against — opponent team, venue, head coach, primary defender, playoff stage, deficit, rest days, post-blowout flag, and health state. Phase 53 fix: when a player has only a free-text <code>player.injury</code> string + <code>injuryRisk</code> (no <code>activeInjury</code> object), the context now infers injury severity from those fields, allowing HEALTH conditions to fire correctly.</div></div>
      <div class="def-card" style="border-left:3px solid #2ecc71;"><div class="def-term">Dynamic Blended Bets <span class="def-abbr">dml/dmargin/dwinner</span></div><div class="def-desc"><strong>Phase 48.</strong> Closures exposed inside <code>renderR2Bets()</code> that interpolate the live ML pick (<code>dml(home, away)</code>), projected margin (<code>dmargin</code>), and named winner (<code>dwinner</code>) from <code>calcBlendedProjection()</code> at render time. Replaces hardcoded "OKC -8.5" strings with <code>{{winner}}</code>/<code>{{margin}}</code> templates. Bet cards always reflect current model state — edits to <code>series-data</code> at 6:55pm show up in the 7pm bet display.</div></div>
      <div class="def-card" style="border-left:3px solid #3498db;"><div class="def-term">Declarative Bet Card Schema <span class="def-abbr">BETS / BET_SLATES</span></div><div class="def-desc"><strong>Phase 55.</strong> Bets migrated from hand-styled HTML cards to a typed data array. Each bet record: <code>{type, pick, odds, facts, chs, confidence, thesis[], narrative, result, betId}</code>. <code>thesis</code> is multi-value (model / matchup / regression / historical / situational / market) since 6+ existing edge labels combined theses. CHS data in its own structured field. <code>confidence</code> enum (lean / coin-flip / medium / high / best-bet) extracted from labels. <code>BET_SLATES</code> stores per-series headers + G1 recap text. Single <code>renderBetCard(bet, dynamics)</code> + <code>renderBetSlateSeries(slate, series, dynamics)</code> covers all 36 cards. bets.js: 3578 &rarr; 3252 lines.</div></div>
      <div class="def-card" style="border-left:3px solid #3498db;"><div class="def-term">Bet Result Outcomes <span class="def-abbr">win/loss/push/void</span></div><div class="def-desc"><strong>Phase 55/56.</strong> A bet record's <code>result</code> field is either <code>null</code> (unresolved live bet) or <code>{outcome, actual}</code>. Outcomes: <strong>win</strong> &check;, <strong>loss</strong> &times;, <strong>push</strong> &equiv;, <strong>void</strong> &oslash; (yellow). Void was added in Phase 56 when Embiid was ruled out morning-of for NYK-PHI G2 — the published Embiid Over 22.5 prop became unresolvable, and voiding it preserved historical accuracy without retroactively deleting the card.</div></div>
      <div class="def-card" style="border-left:3px solid #3498db;"><div class="def-term">Last-Name Fallback Matcher</div><div class="def-desc"><strong>Phase 54.</strong> Defensive widening in <code>calcProjectedBoxScore</code>. Prior-game box scores stored last-name-only entries ("Wembanyama") while rosters used full names ("Victor Wembanyama"). Strict equality match collapsed <code>activePlayers</code> to zero, rendering empty box score tables. Fix: matcher now tries last-name + substring fallback so both formats resolve. Affects all R2 G2+ projected box scores.</div></div>
      <div class="def-card" style="border-left:3px solid #95a5a6;"><div class="def-term">2025 Playoffs Validation Dataset <span class="def-abbr">VAL2025</span></div><div class="def-desc"><strong>May 2026.</strong> <code>playoffs_2025_validation.js</code> — game-by-game results from the 2025 NBA Playoffs (sourced from Basketball Reference) used as an independent validation set for patterns observed in the 2026 R1 model. Was previously untracked in the working tree; now committed alongside <code>test-projections.js</code> for repeatable pattern-checking and backtests without re-pulling from BBRef.</div></div>
      <div class="def-card" style="border-left:3px solid #95a5a6;"><div class="def-term">CHS Test Coverage <span class="def-abbr">TEST 6</span></div><div class="def-desc"><strong>May 2026.</strong> <code>test-projections.js</code> TEST 6 ports the ad-hoc <code>test_scenarios.js</code> debug script (untracked, console-log only, zero assertions) into the formal suite with 19 assertions: compound-scenarios maps include named players (6a), Reaves G1 game context (6b), Reaves CHS fires with negative ptsDelta (6c), Reaves projection includes a Compound modifier label (6d), SGA CHS fires for OKC-LAL G1 (6e), <code>getTeamScenarioSummary</code> returns cascadeEffects (6f), Brunson/Cade/Wemby G1 CHS fires (6g). Suite total: 3447 &rarr; 3466 assertions, 0 failures. Generalized <code>loadGlobals(includeEngine)</code> so engine modules only load for tests that need them.</div></div>
      <div class="def-card" style="border-left:3px solid #2ecc71;"><div class="def-term">Proportional-Multiplicative Factor <span class="def-abbr">PMF</span></div><div class="def-desc"><strong>Phase 51.</strong> Context factors (fatigue, scheme persistence, inconsistency) now scale proportionally with the base talent edge instead of adding fixed points. Formula: <code>adj = |baseMargin| &times; clamped_pct</code>. Fatigue: &plusmn;12%. Scheme Persistence: &plusmn;20%. Inconsistency: 15% compression. On a 15pt margin, max fatigue impact = &plusmn;1.8pts; on a 5pt margin = &plusmn;0.6pts. Additive sources (3PT regression, turnovers, initiators) remain fixed-point.</div></div>
      <div class="def-card" style="border-left:3px solid #2ecc71;"><div class="def-term">Dynamic Rating Adjustments <span class="def-abbr">DRA</span></div><div class="def-desc"><strong>Phase 50.</strong> Per-player effective ratings in <code>calcTeamRating</code>: (a) Injury severity: <code>r &times; (1 - severity &times; 0.25)</code>, (b) Playoff ascension: <code>r + ascension &times; 2.5</code>, (c) Coaching scheme (top-5): <code>r + (adjRating - 5) &times; 0.3</code>. Replaces team-level flat bonuses.</div></div>
      <div class="def-card" style="border-left:3px solid #2ecc71;"><div class="def-term">Win Probability Recalibration <span class="def-abbr">WPR</span></div><div class="def-desc"><strong>Phase 49.</strong> <code>WIN_PROB_SCALE</code> 15&rarr;35, <code>PLAYOFF_UPSET_COMPRESSION=0.18</code>. Produces 55-82% favorite range. Expected series length 5.1 (was 4.2, actual 5.4-5.6). Cross-year validated: underdogs win 41.7% in both 2025 and 2026 R1.</div></div>
      <div class="def-card" style="border-left:3px solid #2ecc71;"><div class="def-term">Resilience Modifier <span class="def-abbr">RES</span></div><div class="def-desc"><strong>Phase 49.</strong> 0-1 score from 5 components (pedigree 30%, coaching 25%, ascension 20%, initiators 15%, bounce-back 10%). Compresses opponent WP toward 50% by up to 10%. Prevents overconfidence against battle-tested underdogs.</div></div>
