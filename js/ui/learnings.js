...
function renderLearningsPage(el) {
  ...
  <div class="page-content">
    ...
    <div class="learning-timeline">
      ...
      <div class="learning-entry milestone">
        ...
        <div class="learning-date">May 5, 2026</div>
        <div class="learning-body">
          <strong>Three-phase model overhaul: (1) WIN_PROB_SCALE recalibrated 15→35 with cross-year validation, (2) per-player dynamic rating adjustments wired into calcTeamRating, (3) multiplicative context architecture replaces fixed additive factors.</strong><br><br>
          <strong>1. WIN PROBABILITY RECALIBRATION (Phase 49):</strong> Old logistic scale=15 produced extreme probabilities (90%+ for most favorites). Cross-year validation: underdogs won 41.7% of all playoff games in BOTH 2025 and 2026 R1. New scale=35 with PLAYOFF_UPSET_COMPRESSION=0.18 produces realistic 55-82% favorite range. Expected series length: 5.1 games (was 4.2, actual 5.4-5.6). Supported by Barreira &amp; Morgado (2023) showing declining HCA and López-García et al. (2024) showing team ability matters more than crowd support.<br><br>
          <strong>2. DYNAMIC RATING ADJUSTMENTS (Phase 50, Option A):</strong> Three per-player adjustments computed inside calcTeamRating weighted sum BEFORE team composite: (a) Active injury severity reduction (r × (1 - severity × 0.25)), (b) Playoff ascension boost (r + ascension × 2.5), (c) Coaching scheme effect for top-5 rotation (r + (adjRating - 5) × 0.3). Old team-level flat ascension bonus removed to prevent double-counting. Edwards example: base 90 + ascension 1.5×2.5 = effective ~93.75.<br><br>
          <strong>3. MULTIPLICATIVE CONTEXT ARCHITECTURE (Phase 51):</strong> Game-context factors converted from fixed additive to proportional-multiplicative. Fatigue: ±12% of |baseMargin| (was ±2pts fixed). Scheme Persistence: ±20% of |baseMargin| (was ±4pts fixed). Inconsistency: multiplicative compression up to 15% (was -2pts fixed). Uses "proportional-additive" pattern for directional factors — adj = |margin| × clamped_pct — preserving correct directionality while scaling with margin size. Independent scoring sources (3PT regression, turnovers, initiators) remain additive.<br><br>
          <strong>4. MARGIN CAP REMOVAL:</strong> Hard cap (±22→±16) removed since WIN_PROB_SCALE=35 fixes root cause of inflated margins. Real NBA margins regularly exceed 30+ (NYK 51pt win, CLE 55pt win).<br><br>
          <strong>5. RESILIENCE MODIFIER:</strong> 0-1 score per team from 5 components (pedigree, coaching, ascension, initiators, bounce-backs) compresses opponent win probability toward 50% by up to 10%. Prevents model from being overconfident against battle-tested underdogs.<br><br>
  ...
  const phaseSummaries = {
    ...
    'Phase 51': 'Win Probability Recalibration + Dynamic Ratings + Multiplicative Architecture — WIN_PROB_SCALE 15→35, PLAYOFF_UPSET_COMPRESSION 0.18, per-player dynamic adjustments (injury severity, playoff ascension, coaching scheme) in calcTeamRating, proportional-multiplicative context factors (fatigue ±12%, scheme ±20%, inconsistency 15% compression) replacing fixed additive values. Margin cap removed. Resilience modifier (0-1) compresses extreme probabilities. Backtest: 87.5% winner accuracy, 11.4pt margin error, avg fav WP 75.0%, series length 5.1 games.',
