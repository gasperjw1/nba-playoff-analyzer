...
const BETS = [
  ...
  // ═══════════════════════════════════════════════════════════════
  {
    ...
    confidence: 'coin-flip', thesis: ['model','market'], narrative: null,
    result: { outcome:'win', actual:'SAS 106-104 — SAS won by exactly 2, so NYK +3 COVERS. The spread-value side the engine flagged is now a PERFECT 5-for-5 in the Finals (NYK +5.5, NYK +2.5, SAS +2.5, SAS +1.5, NYK +3) — and it cashed even though the engine number sat AT the line (SAS by 3) with no inside-the-number cushion, because SAS simply won by less than the number.' },
  },
  {
    ...
    confidence: 'coin-flip', thesis: ['model','market'], narrative: null,
    result: { outcome:'win', actual:'SAS 106-104 — San Antonio wins as a -155 home favorite to force G6. The model winner pick (home/closeout lean) FINALLY lands on the home team, improving to 2-3 in the Finals after a 1-3 start. The Phase 73 elimination amplifier read correctly: a desperate home team 6-1 at Frost Bank played its most controlled game of the series.' },
  },
  {
    ...
    confidence: 'lean', thesis: ['model','historical'], narrative: null,
    result: { outcome:'win', actual:'Total 210 (SAS 106, NYK 104) — UNDER 211.5 cashes. All three San Antonio games went UNDER (200, 209, 210), confirming the venue-backed read: the Frost Bank grind is a real UNDER tailwind while MSG is a coin flip. The Finals total read improves to 4-1.' },
  },
  {
    ...
    confidence: 'lean', thesis: ['model','matchup'], narrative: 'pra-floor',
    result: { outcome:'win', actual:'Wembanyama 33/14/4 = 51 PRA — cleared the 33.5 alt for the FIFTH straight game (40, 41, 46, 38, 51), this time on an efficient 12-22 elimination masterpiece carried by 14 boards + 4 blocks. The single cleanest player floor in the series.' },
  },
  {
    ...
    confidence: 'lean', thesis: ['model'], narrative: 'minutes-secure-floor',
    result: { outcome:'win', actual:'Harper 9/4/4 = 17 PRA — cleared the 9.5 alt for the FIFTH straight game (25, 23, 25, 28, 17) on 28 secure minutes. The minutes-vetted role-PRA floor is now 4-for-4 when authored as a parlay (G2, G3, G4, G5).' },
  },
  {
    ...
    confidence: 'lean', thesis: ['model'], narrative: 'minutes-secure-floor',
    result: { outcome:'win', actual:'Champagnie 8/6/1 + 2 STL = 15 PRA — cleared the 9.5 alt for the FIFTH straight game (27, 13, 18, 13, 15) on 31 secure minutes. The reb/ast/stl-backed role floor held again, the most reliable dimension in the series.' },
  },

  // ═══════════════════════════════════════════════════════════════
  // FINALS G6 — Tue 6/16 at Madison Square Garden (NYK home), NYK leads 3-2
  // NYK can clinch its first title since 1973; SAS faces elimination
  // ═══════════════════════════════════════════════════════════════
  {
    id: 'finals-g6-sas-nyk-spread',
    slate: 'Finals-G6', series: 'SAS-NYK', game: 6, postedAt: '2026-06-16',
    type: 'spread', pick: 'SAS +4.5', odds: '-110',
    facts: [
      {label:'Market',value:'NYK -4.5 (-110)'},
      {label:'Engine margin',value:'SAS by 1 (COIN FLIP)'},
      {label:'Precedent',value:'Spread-value side 5-for-5 (NYK +5.5, NYK +2.5, SAS +2.5, SAS +1.5, NYK +3)'},
    ],
    reasoning: "Engine SAS 107, NYK 106 (SAS by 1) sits ~5.5 points INSIDE the NYK -4.5 market line — the cleanest spread-value setup since G2: the market prices NYK as a clear home favorite to clinch, but the engine sees a coin flip, so SAS +4.5 carries the model's cover edge. The spread-value side has cashed ALL FIVE Finals games (NYK +5.5, NYK +2.5, SAS +2.5, SAS +1.5, NYK +3) even as the raw winner pick went 2-3 — whichever team the engine's margin read puts inside the line covers, every time. SAS +4.5 hits if SAS wins OR loses by ≤4; given the series has produced a single-digit margin in all five games (max margin 10, in G1), a four-point cushion in a projected coin flip is real value. The Phase 71 spread CAUTION + Finals out-of-sample → half-stake lean, but this is the model's highest-conviction G6 read. Bet the points, not the side.",
    confidence: 'lean', thesis: ['model','market'], narrative: null,
    result: null,
  },
  {
    id: 'finals-g6-sas-nyk-ml',
    slate: 'Finals-G6', series: 'SAS-NYK', game: 6, postedAt: '2026-06-16',
    type: 'ml', pick: 'SAS ML vs NYK', odds: '+160',
    facts: [
      {label:'Series',value:'NYK leads 3-2 (can clinch at home)'},
      {label:'Engine',value:'SAS by 1 (elimination amplifier, even on the road)'},
      {label:'Caution',value:'Model winner pick 2-3 in the Finals; market has NYK -190'},
    ],
    reasoning: "Phase 71c engine: SAS 107, NYK 106 (SAS by 1) at MSG — the elimination amplifier bumps a desperate SAS to a razor-thin edge even on the road, so the engine's nominal pick is the Spurs at +160. The LOUD caveat: the model winner pick is only 2-3 in the Finals, the home/elimination lean has whipsawed all series, and the market disagrees sharply (NYK -190, ~65.5% implied). NYK is the rational favorite — clinching at home with the MSG crowd and the Anunoby/Brunson scoring that won G4 — and a +160 SAS ticket is a live-dog dart, not a confident play. The structural case for SAS: a controlled, ball-protecting team that just found its half-court identity (Wemby 33/14/4, Fox 24/9 in G5) and a NYK side that can clinch but has lost twice at Frost Bank. Documented as the engine's nominal SAS lean; the running R4 rule says distrust this pick and lean on the spread (SAS +4.5) — the moneyline is a coin flip the market prices as NYK.",
    confidence: 'coin-flip', thesis: ['model','market'], narrative: null,
    result: null,
  },
  {
    id: 'finals-g6-sas-nyk-total',
    slate: 'Finals-G6', series: 'SAS-NYK', game: 6, postedAt: '2026-06-16',
    type: 'total', pick: 'Under 214.5', odds: '-110',
    facts: [
      {label:'Finals totals',value:'SA: 200, 209, 210 UNDER · MSG: 226 OVER, 213 UNDER'},
      {label:'Engine',value:'OT-adjusted ~214 (a hair UNDER 214.5)'},
      {label:'Venue',value:'MSG split — no clean total edge'},
    ],
    reasoning: "Engine OT-adjusted ~214 sits a hair UNDER the 214.5, but this is the model's WEAKEST G6 read: unlike the Frost Bank UNDERs (all three San Antonio games went under at 200, 209, 210), MSG has SPLIT — G3 ran fast (226 OVER) on the Garden's pace + OG's eruption, G4 went UNDER (213). Back at the Garden the venue gives no clean total edge, so the engine number sitting right at the line makes this close to a true coin flip. The case FOR the UNDER: a projected COIN FLIP / OT-character game is a half-court grind by definition, both defenses have five games of film, and a clinch/elimination game tightens up late. The case AGAINST: MSG ran fast in G3 and a NYK clinch celebration + SAS desperation could push pace. Totals carry the auto-CAUTION pill; quarter-stake at most. The least reliable of the model's G6 reads given the venue split.",
    confidence: 'lean', thesis: ['model','historical'], narrative: null,
    result: null,
  },
  {
    id: 'finals-g6-sas-nyk-wemby-pra',
    slate: 'Finals-G6', series: 'SAS-NYK', game: 6, postedAt: '2026-06-16',
    type: 'prop', pick: 'Wembanyama Over 33.5 PRA', odds: '-125',
    facts: [
      {label:'Finals PRA',value:'40, 41, 46, 38, 51 — all cleared'},
      {label:'Engine',value:'~40 PRA'},
      {label:'Minutes',value:'38-44 every game — max usage in elimination'},
    ],
    reasoning: "Wemby's PRA has cleared 33.5 in ALL FIVE Finals games (40, 41, 46, 38, 51) across every shooting profile, because the REB/FT/scoring composite is rebound-backed and variance-proof — even the cold 9-25 G4 cleared on 13 boards, and the G5 elimination masterpiece (33/14/4) blew it away at 51. For a second straight elimination game, expect MAXIMUM usage and minutes; the central PRA estimate sits ~40 and the 33.5 line is proj-6.5 in the deep-alt zone. The floor travels — it cleared at MSG in G3 (46) and G4 (38), so the venue change is no risk. The single cleanest player floor on the board for the sixth straight game and the anchor of the SAS reliable parlay.",
    confidence: 'lean', thesis: ['model','matchup'], narrative: 'pra-floor',
    result: null,
  },
  {
    id: 'finals-g6-sas-nyk-harper-pra',
    slate: 'Finals-G6', series: 'SAS-NYK', game: 6, postedAt: '2026-06-16',
    type: 'prop', pick: 'Harper Over 9.5 PRA', odds: '-230',
    facts: [
      {label:'Finals PRA',value:'25, 23, 25, 28, 17 — cleared all five'},
      {label:'Minutes',value:'28-32 every game — SECURE bench role'},
      {label:'Engine',value:'~19 PRA'},
    ],
    reasoning: "Harper has cleared 9.5 PRA in ALL FIVE Finals games (25, 23, 25, 28, 17) on 28-32 SECURE minutes — the most consistent role-PRA floor in the series alongside Champagnie, and it cleared at MSG in G3 (25) and G4 (28), so the venue travels. The rookie combo-guard's scoring + rebounding composite is game-script-independent and his bench role only deepens in a second straight elimination game. Pairs with Champagnie as the two minutes-vetted legs of the SAS reliable parlay; both have cleared every Finals game. Engine ~19 PRA; the 9.5 line is proj-9.5 deep. The single most reliable dimension in the series — the role-PRA floor parlay is 4-for-4 when authored (G2/G3/G4/G5).",
    confidence: 'lean', thesis: ['model'], narrative: 'minutes-secure-floor',
    result: null,
  },
  {
    id: 'finals-g6-sas-nyk-champagnie-pra',
    slate: 'Finals-G6', series: 'SAS-NYK', game: 6, postedAt: '2026-06-16',
    type: 'prop', pick: 'Champagnie Over 9.5 PRA', odds: '-280',
    facts: [
      {label:'Finals PRA',value:'27, 13, 18, 13, 15 — cleared all five'},
      {label:'Minutes',value:'30-33 every game — SECURE'},
      {label:'Lesson',value:'Cleared on a 2-9 cold night in G4 — variance-proof'},
    ],
    reasoning: "Champagnie has cleared 9.5 PRA in ALL FIVE Finals games (27, 13, 18, 13, 15) on 30-33 SECURE minutes — including the G4 clear on a frigid 2-9 shooting night, the whole thesis: the rebound/assist/steal composite backstops a cold scoring night for a minutes-secure role player. He cleared at MSG in G3 (18) and G4 (13), so the venue change is no risk. He's the locked-in SAS bench wing whose role deepens in a second straight elimination game. Engine ~14 PRA; the 9.5 line is a deep blowout-stable floor. The second anchor of the SAS reliable parlay and exactly the minutes-secure role-PRA floor the running retro says to PREFER over scoring-dependent star composites.",
    confidence: 'lean', thesis: ['model'], narrative: 'minutes-secure-floor',
    result: null,
  },
];
...
const FEATURED_PARLAYS = [
  ...
  // ─── Finals G5 — Sat 6/13 @ Frost Bank Center (NYK leads 3-1) ─────
  {
    ...
    result: { outcome:'win', delta:'+$135', actual:'ALL THREE legs hit (Champagnie 15 PRA, Harper 17 PRA, Wemby 51 PRA) — the minutes-vetted role-PRA floor wins a FOURTH straight time it was authored (G2 +$135, G3 +$130, G4 +$135, G5 +$135) and has now cleared every leg in all five Finals games. Wemby\'s 33/14/4 anchored an elimination win; the role floors held as designed. The single most reliable structure in the series, now 4-for-4.' },
  },
  {
    ...
    result: { outcome:'win', delta:'+$115', actual:'Both legs hit (Wemby 51 PRA on a 33/14/4 night, Harper 17 PRA on 28 secure min) — the two cleanest cleared-every-game lines delivered again in an elimination win. The highest-floor parlay on the slate cashed exactly as designed.' },
  },
  {
    ...
    result: { outcome:'win', delta:'+$165', actual:'ALL THREE legs hit (UNDER 211.5 on a 210 total, Wemby 51 PRA, Harper 17 PRA) — the correlated SAS-stays-alive grind script played out EXACTLY: a controlled, venue-backed UNDER at Frost Bank with maximum Wemby usage and steady role production, and SAS literally stayed alive forcing G6. The G3-breaking UNDER was correctly trusted back on the Frost Bank grind court. The cleanest correlated stack of the series.' },
  },

  // ─── Finals G6 — Tue 6/16 @ Madison Square Garden (NYK leads 3-2) ─────
  {
    id: 'finals-g6-jun16-floor-sas-role-pra',
    slate: 'Finals-G6', date: '2026-06-16',
    category: 'floor', type: 'best-bet',
    name: 'Finals G6 — SAS Minutes-Vetted Role PRA (3-Leg)',
    stake: 100, odds: '+135', payout: 'To Win: ~$135 (verify Tue evening)',
    legs: [
      { pick:'Champagnie Over 9.5 PRA (alt — Finals G6)', odds:'-280', confidence:'floor', status:null,
        note:'Cleared ALL FIVE Finals games (27, 13, 18, 13, 15) on 30-33 SECURE min — including a 2-9 cold-shooting night and both prior MSG trips (18, 13). Locked SAS bench wing; role deepens in a second straight elimination game. The anchor.' },
      { pick:'Harper Over 9.5 PRA (alt — Finals G6)', odds:'-230', confidence:'floor', status:null,
        note:'Cleared all five (25, 23, 25, 28, 17) in 28-32 SECURE min, including at MSG (25, 28). Combo-guard composite, game-script-independent — travels to the Garden.' },
      { pick:'Wembanyama Over 27.5 PRA (alt — Finals G6)', odds:'-320', confidence:'floor', status:null,
        note:'40/41/46/38/51 PRA — the REB/FT/scoring composite cleared even on a cold 9-25 G4 via 13 boards and blew up at 51 in the G5 elimination win. Max usage in a second elimination game; cleared at MSG in G3 (46) and G4 (38). PRA line, NOT the REB-alt.' },
    ],
    thesis:'The minutes-vetted PRA floor that has cleared EVERY leg in all FIVE Finals games and won the tier each time it was authored (G2 +$135, G3 +$130, G4 +$135, G5 +$135), re-run for a G6 at MSG where the floors have already proven they travel (every leg cleared in G3 and G4 at the Garden). Champagnie (27/13/18/13/15 PRA), Harper (25/23/25/28/17 PRA), and Wemby\'s deep-alt PRA (40/41/46/38/51) are all minutes-secure and rebound-backed — the structure keys the PRA composite and avoids both the standalone REB-alt and the scoring-dependent KAT star-PRA (both broke). The single most reliable structure in the series, 4-for-4 as a parlay. Math: 0.90 × 0.88 × 0.92 ≈ 73% combined; each leg independently in the 80%+ deep-alt zone. Phase 71 R3 Finals stake cap holds at $100.',
    result: null,
  },
  {
    id: 'finals-g6-jun16-floor-two-leg',
    slate: 'Finals-G6', date: '2026-06-16',
    category: 'floor', type: 'best-bet',
    name: 'Finals G6 — Wemby + Harper PRA (2-Leg Tight Floor)',
    stake: 100, odds: '+115', payout: 'To Win: ~$115 (verify Tue evening)',
    legs: [
      { pick:'Wembanyama Over 33.5 PRA (Finals G6)', odds:'-125', confidence:'floor', status:null,
        note:'40/41/46/38/51 PRA all five games. Carried by a guaranteed ~13-reb base, NOT scoring alone — cleared on the cold 9-25 G4. Max usage in a second straight elimination game; cleared at MSG in G3/G4.' },
      { pick:'Harper Over 9.5 PRA (Finals G6)', odds:'-230', confidence:'floor', status:null,
        note:'25/23/25/28/17 PRA on 28-32 SECURE min every game. The most consistent role floor in the series; cleared all five including both MSG trips.' },
    ],
    thesis:'A two-leg tight floor pairing the series\' most variance-proof star composite (Wemby PRA, reb-backed) with its most consistent role floor (Harper, 25/23/25/28/17). Deliberately keys the two cleanest cleared-every-game lines and anchors on guaranteed reb/ast volume + locked minutes per the running retro (avoids the scoring-dependent KAT star-PRA that fell short twice). Both cleared all five Finals games, including at MSG. Math: 0.86 × 0.90 ≈ 77% combined — the highest-floor parlay on the slate. Phase 71 R3 Finals cap → $100.',
    result: null,
  },
  {
    id: 'finals-g6-jun16-trad-value-side',
    slate: 'Finals-G6', date: '2026-06-16',
    category: 'traditional', type: 'best-bet',
    name: 'Finals G6 — Spread Value + PRA Floors (3-Leg)',
    stake: 50, odds: '+360', payout: 'To Win: ~$180 (verify Tue evening)',
    legs: [
      { pick:'SAS +4.5 (Finals G6)', odds:'-110', confidence:'lean', status:null,
        note:'The spread-value side is a PERFECT 5-for-5; engine SAS by 1 sits ~5.5 inside the NYK -4.5 line, and all five Finals games finished single-digit. Covers a SAS win OR a NYK clinch by ≤4. Lead leg.' },
      { pick:'Wembanyama Over 33.5 PRA (Finals G6)', odds:'-125', confidence:'lean', status:null,
        note:'40/41/46/38/51 PRA. A close, contested coin-flip game with max elimination usage correlates with a heavy Wemby composite. Correlated with the SAS +4.5 cover.' },
      { pick:'Harper Over 11.5 PRA (Finals G6)', odds:'-150', confidence:'lean', status:null,
        note:'25/23/25/28/17 PRA — pushed one tick deeper than the floor line. A competitive SAS-keeps-it-close game means the secure bench guard keeps producing. Rounds out the stack.' },
    ],
    thesis:'A CORRELATED stack on the model\'s coherent G6 read at MSG: the spread-value side (SAS +4.5, the series\' single most reliable bet at 5-for-5) plus the two reliable PRA floors. Deliberately AVOIDS the moneyline (the 2-3 Finals winner record + the market\'s NYK -190 clinch lean warn against the side) and the total (MSG split 226/213 gives no venue edge this time, unlike the Frost Bank UNDERs). All three legs move with a competitive, single-digit SAS-keeps-it-close game — the projected coin flip. Math: 0.62 × 0.66 × 0.60 ≈ 25% combined; at +360 (4.6x) ≈ positive EV, lifted by leg correlation. Swaps the venue-dependent UNDER (no MSG edge) for the venue-independent spread-value side. Phase 71 R3 Finals cap → $50 stake; CAUTION pill auto-stamps.',
    result: null,
  },
];
...
const BET_SLATES = {
  ...
  'Finals-G5': {
    label: 'NBA Finals — Game 5 (Archive)',
    games: [
      { series:'SAS-NYK', date:'2026-06-13', time:'8:30 PM ET', venue:'Frost Bank Center', context:'NYK leads 3-1',
        recap:'<strong style="color:var(--green)">Result:</strong> SAS 106-104 — San Antonio staves off elimination and forces G6, cutting NYK\'s lead to 3-2. The Spurs controlled a defensive grind and survived a late NYK push: Brunson\'s potential game-tying three rimmed out at the buzzer. Wembanyama delivered the elimination masterpiece — 33/14/4 with 4 blocks (51 PRA) on 12-22 — and De\'Aaron Fox atoned for his G4 collapse with 24/9 (the closer SAS needed); Vassell added 16 (4-9 3PT) and the minutes-vetted bench floors held a FIFTH straight time (Champagnie 15 PRA, Harper 17 PRA). NYK got 30/9 from Brunson (10-25, Castle held the efficiency again), a bounce-back 23/12 from KAT, and 22 from OG Anunoby (4-9 3PT), but the offense stalled in the half-court and couldn\'t solve Wemby\'s rim protection late. <strong>Model: SAS by 3 → actual SAS by 2 — CORRECT WINNER (the home/closeout read FINALLY lands on the home team; Finals winner pick to 2-3) with a near-exact margin.</strong> Every model read cashed AGAIN: SAS ML WIN, NYK +3 covered (SAS by 2 — the spread-value side is now a PERFECT 5-for-5), UNDER 211.5 hit on a 210 total (all three Frost Bank games went UNDER: 200, 209, 210), and all three PRA floors cleared (Wemby 51, Harper 17, Champagnie 15). Bets 6-0; parlays 3-0 (+$135 floor / +$115 floor / +$165 trad = +$415) — tied for the slate\'s best night of the Finals. The cleanest model alignment of the series: winner, spread, total, AND every prop. <span style="color:var(--text-dim)">— Original preview:</span> Back in San Antonio with NYK one win from its FIRST TITLE SINCE 1973 — the Knicks lead 3-1 after the largest comeback in Finals history. <strong>The stakes:</strong> a NYK win CLINCHES the championship on the road; a SAS win forces G6 back at MSG and keeps the comeback story alive. <strong>The model read:</strong> the engine flips back to San Antonio — SAS by 3 (108-105), character COMPETITIVE — on the home/closeout dynamics: SAS is 6-1 at Frost Bank this playoff run and the Phase 73 elimination amplifier is now ACTIVE (a desperate home team down 3-1 widens variance and earns a desperation bump). But the running rule holds: the raw winner pick is only 1-3 in the Finals, so weight the margin/spread (the spread-value side is a PERFECT 4-for-4) over the side — with SAS favored at home, NYK getting the points is likely the model value side. <strong>The swing:</strong> SAS\'s elimination desperation + the 6-1 home edge vs a NYK team playing with house money behind the Anunoby/Brunson scoring that won G4 and a bounce-back KAT. The total leans UNDER again (~209) on the Frost Bank grind pace (both SA games went UNDER: 200, 209). DK (6/12): SAS -3 / 211.5 / ML SAS -155, NYK +135. Engine: SAS by 3 — the spread sits near fair, UNDER 211.5 the thin total lean. <strong>Phase 71 R3 out-of-sample stake cap stays 50%</strong>; CAUTION pill auto-stamps. Finals stakes ($100 floor / $50 trad) hold.' },
    ],
  },
  'Finals-G6': {
    label: 'NBA Finals — Game 6 (Live)',
    games: [
      { series:'SAS-NYK', date:'2026-06-16', time:'8:30 PM ET', venue:'Madison Square Garden', context:'NYK leads 3-2',
        recap:'<strong style="color:var(--purple)">Finals G6 Preview (Tue 6/16):</strong> Back at Madison Square Garden with New York one win from its FIRST TITLE SINCE 1973 — NYK leads 3-2 after San Antonio extended the series with a controlled 106-104 elimination win in G5. <strong>The stakes:</strong> a NYK win ENDS it (championship at home, on a clinch); a SAS win forces a winner-take-all Game 7 back at Frost Bank Center. <strong>The model read:</strong> the engine produces its tightest projection of the series — SAS by 1 (107-106), character COIN FLIP — because the Phase 73 elimination amplifier bumps a desperate SAS even on the road. But the running rule holds: the raw winner pick is only 2-3 in the Finals and the home/elimination lean has whipsawed all series, so weight the margin/spread (<strong>the spread-value side is a PERFECT 5-for-5</strong>) over the side. With the engine putting SAS by 1 deep inside the market NYK -4.5, <strong>SAS +4.5 is the clean value side</strong>. <strong>The total</strong> is a coin flip at MSG (the Garden split 226 OVER / 213 UNDER while all three San Antonio games went under), engine ~214 vs the 214.5 line — no venue edge. <strong>The reliable threads:</strong> the SAS role-PRA floors (Wemby, Harper, Champagnie) have cleared EVERY Finals game including both prior MSG trips, so they travel and anchor the reliable parlay regardless of who wins. <strong>The swing:</strong> NYK\'s clinch poise + the MSG crowd vs a desperate Wembanyama-led SAS that just found its half-court control (33/14/4 in G5). DK opener (6/15): NYK -4.5 / 214.5 / ML NYK -190, SAS +160. <strong>Phase 71 R3 out-of-sample stake cap stays 50%</strong>; CAUTION pill auto-stamps. Finals stakes ($100 floor / $50 trad) hold.' },
    ],
  },
};
