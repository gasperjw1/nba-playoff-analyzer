...
const FEATURED_PARLAYS = [
  ...
  // ─── R2 G4 — TONIGHT (Sun May 10: NYK-PHI G4 + SAS-MIN G4) ─────────────
  // RELIABLE FLOOR PARLAYS — each leg ≥80% per-leg hit rate.
  {
    ...
    legs: [
      { pick:'Wemby Over 11.5 rebounds (alt — SAS-MIN G4)', odds:'~-300', confidence:'floor', status:null,
        note:'15reb in EVERY series game. Counting stat, blowout-proof. Real hit rate ~95%.' },
      { pick:'Wemby Over 2.5 blocks (SAS-MIN G4)', odds:'-140', confidence:'floor', status:null,
  ...
  },
  {
    ...
    stake: 100, odds: '+50', payout: 'To Win: ~$50 (verify on DK)',
    legs: [
      { pick:'KAT Over 8.5 rebounds (alt — NYK-PHI G4)', odds:'~-220', confidence:'floor', status:null,
        note:'10reb G2, 12reb G3 — PHI C rotation thin (Drummond + Bona + hobbled Embiid). Real hit rate ~85%.' },
      { pick:'Brunson Over 24.5 points (alt — NYK-PHI G4)', odds:'~-220', confidence:'floor', status:null,
  ...
  // TRADITIONAL (value plays — engine edge, lower combined hit rate)
  {
    ...
    legs: [
      { pick:'SAS ML vs MIN (G4)', odds:'-195', confidence:'high', status:null,
        note:'Model: SAS by 3 (~65%). G3 win at MIN proved road durability with Wemby in transcendent form.' },
      { pick:'Wemby Over 12.5 rebounds (SAS-MIN G4)', odds:'-140', confidence:'floor', status:null,
  ...
  },
  {
    ...
    legs: [
      { pick:'NYK ML vs PHI (G4)', odds:'-122', confidence:'medium', status:null,
        note:'Sweep-cliff closeout. Model ~63% vs market ~55%.' },
      { pick:'Brunson Over 27.5 points (NYK-PHI G4)', odds:'-115', confidence:'high', status:null,
  ...
  },
  {
    ...
    legs: [
      { pick:'PHI ML vs NYK (G4)', odds:'+102', confidence:'chaos', status:null,
        note:'Last home game; Embiid back; Maxey/George 40+min in desperation. Closeout backdoor scenario.' },
      { pick:'MIN ML vs SAS (G4)', odds:'+162', confidence:'chaos', status:null,
  ...
  'R2-G4': {
    ...
    games: [
      { series:'NYK-PHI', date:'2026-05-10', time:'3:30 PM ET', venue:'Wells Fargo Center', context:'NYK leads 3-0',
        recap:'<strong style="color:var(--purple)">G4 Setup (closeout):</strong> NYK leads 3-0 after winning G3 108-94 in PHI. Brunson 33pts/9ast G3 (24th career 30+ playoff game), George 0-9 after Q1 historic disappearance. PHI faces sweep at home with Embiid PROBABLE (hip only — ankle off the report) and OG Anunoby QUESTIONABLE for NYK (hamstring strain, missed G3). DK: NYK -1.5 / total 213.5 / NYK -122 / PHI +102 — much tighter than the engine baseline (NYK by 4). Closeout dynamics: favorites win ~70% of G4 sweeps but cover only ~50% ATS — backdoor losses common. PHI desperation effect is REAL but capped (~+3) — model knows from G3 that PHI\'s IC=2 ceiling caps offensive output around 100-105.' },
      { series:'SAS-MIN', date:'2026-05-10', time:'7:30 PM ET', venue:'Target Center', context:'SAS leads 2-1',
        recap:'<strong style="color:var(--purple)">G4 Setup:</strong> SAS leads 2-1 after stealing G3 115-108 at Target Center behind Wemby\'s 39/15/5blk Kareem-tier line. Edwards roared back with 32/14/6 (no minutes restriction confirmed for G4 per Wolves beat). MIN must hold serve to send series back to SAS tied 2-2. DK: SAS -4.5 / total 218.5 — market is MORE bullish on SAS than the engine (says SAS by 3). Total moved up from G3\'s 215.5 → 218.5 reflecting both teams\' G3 efficiency. Lean: model prefers SAS ML at -195 over the rich -4.5 spread; Wemby variance and Edwards-led MIN home desperation make backdoor possible.' },
