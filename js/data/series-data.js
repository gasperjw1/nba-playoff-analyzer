...
const SERIES_DATA = [
  ...
  // ============================================================
  {
    ...
    games: [
      ...
      {num:3,result:null,homeScore:null,awayScore:null,winner:null,prediction:{homeWin:true,homeScore:114,awayScore:104,margin:10,confidence:"MEDIUM",character:"COMPETITIVE",keyTakeaways:[
  ...
  "<strong>Why SAS by 9 (engine baseline):</strong> Phase 58 engine fix now applies HCA flip correctly (G3 venue at MIN). Engine projects SAS by 9 (vs SAS by 11 pre-fix) — the baseline already accounts for HCA flip, MIN's home crowd, and the round-scaled HCA value of 1.7. Tactical add-ons (MIN desperation after worst playoff loss, Edwards potentially expanded) could compress this another 3-5pts toward MIN. PRIMARY HEDGE: Edwards minutes — 32+ at G1 efficiency (8-13, 18pts/25min) flips this to MIN by 2-3.",
    ...
    ],reasoning:"G4 Sunday May 10 at Target Center. SAS leads 2-1 after a road win in G3. MIN must hold serve to send series back to SAS tied 2-2. Engine baseline SAS by 5; HCA flip with trimmed premium (1.5pts vs the 2.0 baseline that overshot in G3) gives SAS by 3. Edwards proved fully healthy G3 (32pts/14reb/6ast in 41min) — upgrades MIN's IC to ~2.0 stars. But Randle/McDaniels combined 8-34 G3 = secondary scoring crisis persists. SAS 107, MIN 104 final. Verify DK lines morning-of (likely SAS -1.5 to -3.5 / total 215-220). The G3 retro confirmed HCA flip + 'home desperation' was previously overcalibrated; G4 prediction now uses a tighter 1.5pt HCA premium and a +2 desperation bump (instead of the +3-4 the earlier model implied). Wemby is the swing factor: transcendent form sustained = SAS by 5+; reverts to mean = SAS by 0-2 / coin flip. Edwards full minutes = MIN ceiling rises 4-6 (still not enough to flip). MIN must combine Wemby off-night + Edwards 30+ + 3PT variance to win. That's not a high-probability conjunction.",xFactor:"Wemby variance + Edwards minutes/health + 3PT volatility",moneyline:"SAS -150 / MIN +130 (est, verify on DK)",spread:"SAS -2.5 (est)",ou:"O/U 217.5 (est)"},notes:""}]
,
    modelLessons: [
  ...
  "<strong>Risks:</strong> (a) OG Anunoby's unspecified G2 issue — if OUT, NYK's wing depth thins from 4 to 3 and PHI hunts Bridges on Maxey. (b) NYK shooting regression: G1 63% → G2 51% → expected 47-49% G3 as PHI's home defense tightens. (c) PHI fatigue if Embiid plays 35+ on a compromised ankle.",
    ...
    ],reasoning:"G4 Sunday May 10 at Wells Fargo Center. NYK leads 3-0 — closeout game. PHI's last stand at home before the season ends. Engine baseline NYK by 8-9; HCA flip compresses to NYK by 5-6; PHI home desperation bump trims to NYK by 4. Confidence MEDIUM because the 'down 0-3 dynamic' is well-charted (favorites win ~70% of G4 closeouts but cover only ~50% ATS — backdoor losses by underdogs in clinchers are common). Verify DK lines morning-of: market is likely NYK -2.5 to -4.5 / total 211.5-214.5 depending on Embiid + OG status. PHI desperation effect is REAL but capped (~+3) — model knows now from G3 that PHI's IC=2 ceiling caps their offensive ceiling around 100-105. NYK structural edges (Brunson, KAT, depth, +9.2 clutchNetRtg) keep them favored.",xFactor:"Embiid minutes/effort + OG return + NYK closing-game intensity",moneyline:"NYK -160 / PHI +135 (est, verify on DK)",spread:"NYK -3.5 (est)",ou:"O/U 213.5 (est)"},notes:""}]
,
    modelLessons: [
