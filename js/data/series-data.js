...
const SERIES_DATA = [
  ...
  },
  {
    ...
    game3: {
      spread: "POR -1.5", moneyline: "SAS -125 / POR +105", ou: "O/U 213.5",
      pick: "SAS", confidence: "low", projScore: "SAS 108 — POR 105",
      schedule: "Fri Apr 24 — 10:00 PM ET — Prime Video",
      reasoning: "PHASE 30 CONDITIONAL PICK: SAS (LOW confidence). This prediction is highly conditional on Wemby's concussion protocol clearance.\n\nWEMBY STATUS: Entered concussion protocol at 8:57 Q2 in G2 after face-first fall. NBA requires minimum 48 hours + independent neurologist clearance. G3 is Fri Apr 24 — barely within the 48-hour window. His status is UNCERTAIN, likely a game-time decision. Historical data: same-series returns from concussion protocol are uncommon.\n\nIF WEMBY PLAYS (60% probability): SAS wins by ~8. His +16.9 on/off means SAS is elite WITH him and mediocre WITHOUT. Even at reduced efficiency (concussion effects on vision/balance → project 40-42% FG vs 52% season), his presence alone transforms the defense and creates gravity. Pick: SAS by 8, MEDIUM confidence.\n\nIF WEMBY OUT (40% probability): POR wins by ~5. SAS collapsed in G2 without Wemby (outscored by 10 in his absence). POR at home with Henderson's youth breakout momentum, Holiday's veteran bounce-back, and Avdija's star play makes them favorites. Pick: POR by 5, MEDIUM confidence.\n\nBLENDED PREDICTION: 0.6*(SAS+8) + 0.4*(POR+5) = SAS by ~2.8 ≈ SAS by 3. But uncertainty is massive, so LOW confidence.\n\nYOUTH BREAKOUT WATCH: Scoot Henderson (age 21) is THE breakout story. 18pts G1 → 31pts G2 (11-17 FG, 5-9 3PT). Phase 29 youth breakout persistence: consecutive breakout games for ≤23-year-olds momentum persists. At HOME in Portland, expect another 25+ game. Henderson is now POR's best player by production, not just potential. Also watch Dylan Harper (SAS, age 20, rookie) who had 10pts/4ast in G2 with expanded minutes after Wemby exit.\n\n3PT VARIANCE: SAS shot 29% from 3 in G2 (7-24) — regression UP expected (~35%). POR shot 41% from 3 in G2 — regression DOWN expected (~34%). Net: minimal swing. Henderson's 5-9 3PT was the outlier — expect ~3-5 3PT in G3.\n\nTURNOVERS: SAS had 9 TOs in G2 (low, disciplined), POR had 12. On the road, SAS TOs could increase slightly (+1-2). Not a major factor.\n\nROLE FLEXIBILITY: SAS 8.0 vs POR 4.0. SAS's flexibility advantage shrinks significantly without Wemby — he IS the flexibility (C/PF/wing defender). Without him, SAS is a conventional team.\n\nCOACHING: Johnson (adj 5) vs Splitter (adj 4). Neither coach has shown dominant adjustment ability. Series will be decided by talent (Wemby status) and player performance, not coaching.\n\nREALISTIC SHOOTING: SAS ~44% FG / 35% 3PT. POR ~43% FG / 34% 3PT. Individual: Fox ~43%, Castle ~40%, Henderson ~48% (home boost), Holiday ~42%, Avdija ~45%.",
      g2Adjustments: [
      ...
      prosHome: ["IF WEMBY PLAYS: His +16.9 on/off transforms SAS from mediocre to elite. Even at reduced efficiency, his presence = SAS favored.", "Castle/Fox stepped up without Wemby — 35pts/14ast combined in G2. System offense works regardless.", "SAS 3PT regression UP from 29% → ~35% expected", "SAS still has better overall talent even without Wemby — Fox/Castle/Vassell core is solid", "Harper (age 20) youth breakout potential with expanded minutes"],
      consHome: ["WEMBY STATUS UNKNOWN — if OUT, SAS is an underdog at Portland. Series changes completely.", "ROAD GAME at Moda Center — POR crowd electric after G2 upset", "Henderson's youth breakout momentum (18→31pts) will carry to home floor", "Holiday found rhythm in G2 (19/9/5) — veteran presence stabilizes POR", "Sharpe bounce-back (11pts/9reb/5ast G2 on 5-6 FG) adds a 3rd scorer for POR", "Even WITH Wemby — concussion effects may reduce his efficiency (vision, balance, aggression)"],
      prosAway: ["HOME COURT — Moda Center, first home game. POR crowd electric after stealing G2.", "Henderson YOUTH BREAKOUT at home (age 21): 18→31pts trending. Phase 29 persistence = expect 25+.", "Holiday veteran rhythm (19/9/5 G2) + home comfort = potential 20+ game", "Avdija is POR's star (22pts avg through 2 games) — at home, expect 24+", "Sharpe found rhythm: 5-6 FG G2 after 4-13 G1. Trending right direction.", "IF WEMBY OUT: POR becomes clear favorite at home. Series completely transforms."],
      consAway: ["IF WEMBY PLAYS: Even diminished Wemby is the best player in the series by far. His gravity opens everything for SAS.", "POR still has -1.0 NetRtg — structurally weaker team on paper", "Clingan (4-12 G2) still has no answer for SAS length even without Wemby", "Henderson's 64.7% FG in G2 WILL regress — expect ~44-48% in G3", "Coaching gap remains: Johnson (adj 5) > Splitter (adj 4), even on the road", "POR's turnover issues (12 in G2) could worsen under home pressure (pressing)"]
    },
    coaching: {
      home: {
        coach: "Mitch Johnson",
    ...
    // ── G3 Per-Player Offensive Outlook (Phase 30 — CONDITIONAL on Wemby status) ──
    // Series tied 1-1. G3 at Moda Center (POR home). Wemby concussion protocol — game-time decision.
    // IF WEMBY PLAYS: SAS favored by ~8. IF OUT: POR favored by ~5. Blended: SAS by ~3.
    ...
    g3PlayerOutlook: {
      home: [ // SAS (AWAY at Moda Center for G3)
        // Wemby: CONCUSSION PROTOCOL. 5pts/4reb/1blk in 12min G2 before exit.
        // G1: 35pts (13-21, 5-6 3PT). Season ~47% FG. IF HE PLAYS: reduced efficiency.
        // Concussion effects: vision, balance, aggression all potentially impaired.
        { player:"Victor Wembanyama", outlook:"neutral", projFgPct:0.42, ptsRange:[18,28], reason:"CONDITIONAL — game-time decision (concussion protocol). IF PLAYS: expect reduced efficiency from G1's 61.9% FG. Concussion after-effects (vision blur, balance, reduced aggression) project 42% FG — below his 47% season average. Even diminished, his +16.9 on/off means SAS transforms from mediocre to elite just by his presence. His gravity alone opens everything for Fox/Castle. At 42% FG, project 20-24pts on lower volume with continued rim protection. IF OUT: N/A — SAS becomes a fundamentally different team. Recovery volatility flag at maximum.", confidence:"low" },

        // Fox: G1 17pts/6ast, G2 17pts/6ast (6-16 FG, 37.5%). Season ~47% FG.
        // Consistent 17pt production but inefficient. Road game.
        // Bayesian: 0.40*(~0.40 avg) + 0.30*(0.375 G2) + 0.30*(0.47 season) = ~0.42. Road → 0.41
        { player:"De'Aaron Fox", outlook:"neutral", projFgPct:0.41, ptsRange:[16,24], reason:"Rock-solid 17pts/6ast in BOTH games — consistent if not efficient (37.5% G2). Road game at Moda Center adds difficulty. If Wemby plays, Fox benefits from reduced attention. If Wemby is OUT, Fox must become THE scorer — expect 22-28pts on higher volume but similar efficiency. Bayesian blend projects 41% FG road game. His speed in transition is venue-independent but half-court creation suffers without Wemby's gravity.", confidence:"medium" },

        // Castle: G1 17pts, G2 18pts/5reb/8ast/3stl (7-20 FG, 35%). Season ~44% FG.
        // Consistent production but volume shooter. Road game.
        // Bayesian: 0.40*(~0.40 avg) + 0.30*(0.35 G2) + 0.30*(0.44 season) = ~0.40. Road → 0.39
        { player:"Stephon Castle", outlook:"neutral", projFgPct:0.39, ptsRange:[14,22], reason:"Averaging 17.5pts/6.5ast through 2 games — productive despite 35% G2 FG. Expanded playmaking role (8ast G2) shows he can create for others. Road game adds difficulty for a young player. If Wemby OUT, Castle's ball-handling becomes critical — expect heavy usage. His defensive versatility (3stl G2) travels to any venue. Efficiency may stay low (~39% FG) but volume keeps scoring up.", confidence:"medium" },
