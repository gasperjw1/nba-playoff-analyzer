# FATIGUE AGENT — Round 1 Essay (v2 — re-run with strict lane discipline)
...
2. **Wemby's G1 shot-quality decay pattern is NOT generic "off night" — it has a fatigue signature.** Yahoo Sports / Sports Illustrated post-G1 analysis: Wemby "missed 12 of 16 attempts over the final three quarters" and "the Knicks outshot the Spurs 40.9% to 28.6% in the fourth quarter" [WebSearch + sourced; Yahoo Sports URL: https://sports.yahoo.com/nba/article/nba-finals-why-victor-wembanyama-looked-gassed-against-the-knicks-at-the-end-of-game-1-loss-045810247.html, Tier 2 verified]. The qualitative-signal database confirms the Q4 mechanism: "Three back-to-back-to-back missed shots when score was 94-89... Costly turnover at 99-95. WORST stretch came in clutch minutes when fatigue compounds with pressure." [`js/data/qualitative-signals.js`, severity 0.85, Tier 1, `nextGameImplication: 'wemby-clutch-cap'`]. Per `signal-calibration.js`, qualitative-observation weight = 0.45.
...
6. **Mitchell Robinson injury status is a SECOND durability observation worth flagging.** Per NBC Sports / NBA.com / Heavy: Robinson played G1 13 min, 1-2 FG / 2 pts / 6 reb post-surgery (June 3 surgery on fractured right fifth metacarpal). Listed PROBABLE for G2. [WebSearch Tier 2 — https://heavy.com/sports/nba/new-york-knicks/mitchell-robinson-injury-update-game-2/]. In fatigue terms: Robinson is a 13-min role player whose hand surgery limits him to short bursts — meaning KAT absorbs most of the C-minute load. KAT played 34 min G1; expect 32-36 min G2. This compresses NYK's frontcourt rotation. Net effect on fatigue cliff: small relative to SAS's backup-C scarcity but worth flagging as a sub-thesis risk.
...
- **KAT:** pts 16-24 (proj 19), reb 9-13 (proj 11), ast 2-5 (proj 4). **PRA 30-40.** G1: 34 min, 18/12. With Robinson minutes capped by hand injury (13 min G1 likely-repeats), KAT absorbs the structural C-load. Lean OVER PRA 28.5.
...
- Tier 1 (local): `js/data/series-data.js` G1 box scores + modelLessons; `js/data/external-research.js` entries 1, 3, 4, 7; `js/data/qualitative-signals.js` Wemby Q4 cliff; `js/data/signal-calibration.js` weights.
- Tier 2 (WebSearch verified): Yahoo Sports Wemby Q4 analysis URL; Yahoo Sports + Bleacher Report Brunson injury URLs; Heavy.com Mitchell Robinson G2 status URL.
- Tier 3-5: not used.
...
- **ADDED** Wemby's specific Q2-Q4 shot decay pattern ("12-of-16 missed over the final three quarters; 40.9% vs 28.6% Q4 shooting") — a fatigue-mechanism signature, not generic "off night."
- **ADDED** Wemby defensive coverage load (Yahoo Sports observation) as a compounding fatigue mechanism on top of offensive load.
- **ADDED** Mitchell Robinson hand-injury status (probable, 13 min G1) as a secondary durability observation affecting NYK frontcourt rotation.
